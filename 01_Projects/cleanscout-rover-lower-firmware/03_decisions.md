...
## Decision: mechanical-arm command layer is the bus-servo ASCII text protocol, not raw PWM semantics
...
During the first OrangePi visual-tracking prototype, the driver used the historical name `yh_pwm_text` because the
vendor command looks like `#000P1500T1000!`. After reading the later-provided bus-servo documentation, this name can
mislead future engineers into thinking `P1500` means direct PWM waveform output.

### Decision

Treat `#IDPxxxxTxxxx!` as the mechanical-arm bus-servo ASCII command layer. In the current STM32 baseline, that text
layer can be consumed locally by `pwmServo_*` output code and also forwarded over UART to bus devices. Future tasks
must distinguish:

- command protocol: bus-servo ASCII text frames
- execution backend: local PWM generation, UART bus forwarding, or both
- reference-only protocols: the ROS2-style `0x80/0x90` binary frames are not the current verified default

### Consequences

- Positive: C-5.0.x mechanical-arm work can reuse the correct text command form without inventing a new protocol.
- Positive: future docs should not describe `yh_pwm_text` as raw PWM control.
- Negative: the next hardware task still must confirm whether the physical arm is wired as local PWM outputs or a UART bus chain.
...
- `OrangePi/rk3588_ai/arm_tracking_demo/arm_driver.py`

## Decision: mechanical-arm first-stage tracking should be fixed-pose local visual servo, not per-frame full IK
...
The mechanical-arm baseline exposes a real inverse-kinematics helper in `y_kinematics`, but that code only covers the main arm chain and the current OrangePi tracking reference already points to a smaller-joint visual-servo style, not a full per-frame arm solve. The user also clarified the physical servo layout: `000~005` bottom-to-top, with `004` as front rotation and `005` as the gripper.

### Decision

For C-5.0.x mechanical-arm tracking, the first working stage should be:

- move the arm into a safe forward-facing neutral pose first
- use target-center error to adjust only a small joint subset inside a limited window
- keep `004/005` out of the tracking loop in stage 1
- reserve full IK for later pose placement, end-effector staging, or grasp preparation

Do not describe the first stage as "every frame full IK tracking". That wording is wrong for the current baseline and misleads future handoff work.

### Consequences

- Positive: first tracking demos stay simple, inspectable, and less likely to fight the arm into unstable poses.
- Positive: docs can clearly separate tracking, pose move, and grasp phases.
- Negative: any later grasp-capable version still needs a dedicated IK/pose layer and calibration work.
