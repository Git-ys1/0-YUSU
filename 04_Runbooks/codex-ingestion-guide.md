...
- 各项目 Codex 可以在完成项目复盘后把记忆文件写进本 vault。
- 管理员 Codex 在本管理仓库里负责验收这些改动：检查证据、路由、敏感信息、乱码、格式、索引和 `Memory Routing Audit`。
- 验收通过后，管理员 Codex 可以在本仓库统一 stage、commit、push。不要等项目 Codex 自己来本管理仓库提交。
- 管理员可以修正入库质量问题，但不要凭空改写项目事实；证据不足的内容标为 `pending` 或退回到项目负责人补充。
