...
- UI shell state on 2026-06-21: `07_PersonalSite/marginalia-ui` uses the YUSU dark workbench palette by default, desktop keeps a source-native side rail, and mobile uses a bottom nav so `/marginalia/*` pages are full-width rather than a squeezed embedded frame. Build with `tools/build-yusu-integrated-marginalia-ui.ps1` after source UI edits.
