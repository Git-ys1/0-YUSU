...
- Ingest state on 2026-06-19: `sync-yusu-kb-to-marginalia.ps1 -Check` reports `in_sync=194`, `new=0`, `modified=0`, `missing=0`, `moved=0`.
- Semantic index state on 2026-06-19: `semantic-index/default` built with 179 entries, `BAAI/bge-m3`, 1024 dimensions.
- Local UI/API state on 2026-06-19: API `http://127.0.0.1:8000`, UI `http://127.0.0.1:5173`, DeepSeek LLM route `https://api.deepseek.com`, embedding shim `http://127.0.0.1:8011/v1`.
- Personal-site integration on 2026-06-19: `07_PersonalSite` exposes a full-page Agent console at `http://127.0.0.1:8787/#agent` and a near-full-viewport Marginalia workspace at `http://127.0.0.1:8787/#marginalia`.
...
| 8787 | YUSU Personal Site | Embeds live Markdown search, `/api/doc`, full-page Agent chat, and a dedicated Marginalia UI workspace. |
...
If the DeepSeek key or model changes, rerun `tools/configure-marginalia-deepseek.*`, restart `run-marginalia-api-yusu.*`, smoke `/api/marginalia/status`, then run ingest/reindex if Markdown has changed.

2026-06-19 live status:

- `http://127.0.0.1:8787/api/marginalia/status` works and reports DeepSeek `deepseek-v4-flash`.
- Marginalia API `http://127.0.0.1:8000`, UI `http://127.0.0.1:5173`, and BGE shim `http://127.0.0.1:8011` were verified running.
- `sync-yusu-kb-to-marginalia.ps1 -Check` reports `in_sync=194`, `new=0`, `modified=0`, `missing=0`, `moved=0`.
- Semantic index manifest reports 179 entries, `BAAI/bge-m3`, 1024 dimensions.
