...
- Ingest state on 2026-06-08: 146 active file entries in DB, all `ingest_status=done`.
- Semantic index state on 2026-06-08: `semantic-index/default` built with 146 entries, `BAAI/bge-m3`, 1024 dimensions.
- Local UI/API state on 2026-06-08: API `http://127.0.0.1:8000`, UI `http://127.0.0.1:5173`, LLM shim `http://127.0.0.1:8010/v1`, embedding shim `http://127.0.0.1:8011/v1`.
...
Actual ingest requires a working LLM profile in `.marginalia-yusu/.env`. On this machine the intended route is the local LLM shim:
...
If the CLI proxy LLM route returns `502 unknown provider`, do not use Marginalia chat/agent as evidence. Continue with `rg`, `tools/search-kb.*`, Marginalia metadata search, and direct semantic recall until the LLM route recovers.
