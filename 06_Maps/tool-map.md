...
| Enterprise WeChat intelligent bot long connection | WeCom bot integration | official doc path `101463`; endpoint `wss://openws.work.weixin.qq.com`; subscribe with `aibot_subscribe` body `bot_id`/`secret`; heartbeat `ping`; reply via `aibot_respond_msg` and callback `req_id` |
...
| Keil MDK | STM32 firmware build | keep Keil-first workflow where the repo already depends on it, but do not trust historical build logs as proof the current machine can still compile; rediscover the real `UV4.exe`, audit `pCCUsed`, and replace personal CMSIS-DSP absolute paths with repository-relative official dependencies in an isolated workspace |
| STM32CubeProgrammer | STM32 flashing | canonical Windows CLI: `F:\AcademicHub\STMicroelectronics\stm32cubeprogrammer\bin\STM32_Programmer_CLI.exe`; check this exact path before search, never recursively scan whole drives; use a local flash wrapper when available, do a read-only board identity check for imported baselines, then verify runtime behavior after flash |
...
| SheetJS | browser spreadsheet import | Apache-2.0 `xlsx.full.min.js` is vendored under `07_PersonalSite/web/vendor/sheetjs/`; `/routine/` uses browser-side parsing for Tomato ToDo `.xls` to avoid backend Excel COM/xlrd Chinese mojibake |
| Tomato ToDo routine tracker | exam-prep habit analytics | `/routine/` uploads `.xls/.xlsx/.csv/.tsv`, stores originals and merged JSON under ignored `07_PersonalSite/local/routine/`, and exposes `/api/routine/stats` plus optional DeepSeek summary |
...
| SOLIDWORKS 2024 COM/API | Windows parametric CAD automation | use early-bound `SolidWorks.Interop.sldworks.dll`; prefer native sketch + extrusion and multi-contour profiles over unstable dynamic COM / `CreateBodyFromBox3` array marshaling; validate `AddComponent5` placement with component bounding boxes |
